...
The course server contains framework-independent authentication and Course Agent orchestration, PostgreSQL adapters, command-line entry points, and the Phase 4 FastAPI transport. FastAPI remains at the outer adapter boundary and is not imported by `agent_core`.
...
- `course_server.agent_cli` is the Phase 3 model-backed smoke-test entry point.
- `course_server.api` is the Phase 4 HTTP application factory and production entry point.
